...
void mainmenu (void) {
	...
	do {
		...
		drawboard();
		pageflip(); setpagemode(0);
		domenu (menu1,"PLISDHCOE\x05",1,9,1,24,11,8,24,1);

		setpagemode(0);
		if (key==5) {				// ctrl-e
			setpagemode(1);
			clrvp (&mainvp,248); drawboard();
			pageflip(); setpagemode(0);
			design();
			statwin(0);
			}
...
//				cheatchar=0; getkey(); pixwrite (key-'0'); swrite=1;
//				};
			switch (key) {												// COMMANDS
				case 'R': recordmac ("temp_mac"); break;
				case 'E': macrecend(); break;
